...
for i in range(len(via_points)):
    temp_history = robot.ik_jacobian(
        angle_history[-1], via_points[i], max_iteration=1000
    )
...
def animate(i):
    ...
    if i > 0:
        ...
        end_effector_line_list[i].set_data(
            [from_point[0], to_point[0]], [from_point[1], to_point[1]]
        )
